...
public:
     ...
     */
    std::vector<WPointXYZId> getPoints(unsigned int xCells, unsigned int yCells, unsigned int zCells);
...
private:
     ...
     */
    template <typename T>
    WPointXYZId calculateIntersection(const std::vector<T> *vals,
...
template <typename T>
std::shared_ptr<WTriangleMesh> WMarchingCubesAlgorithm::generateSurface(size_t nbCoordsX, size_t nbCoordsY, size_t nbCoordsZ,
                                                                        const WMatrix<double> &mat,
                                                                        const std::vector<T> *vals,
                                                                        double isoValue,
                                                                        std::shared_ptr<WProgressCombiner> mainProgress)
{
    ...
    std::cout << "Number of points for base mar ching cubes: " << points.size() << std::endl;
    ...
    for (const auto &point : points)
    {
        ...
        if (wMarchingCubesCaseTables::edgeTable[tableIndex] != 0)
        {
            ...
            {
                WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 3);
            ...
            {
                WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 0);
                unsigned int id = getEdgeID(point.x, point.y, point.z, 0);
                m_idToVertices.insert(ID2WPointXYZId::value_type(id, pt));
            }
            if (wMarchingCubesCaseTables::edgeTable[tableIndex] & 256)
            {
                WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 8);
            ...
            if (point.x == m_nCellsX - 1)
            {
                ...
                {
                    WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 2);
                    unsigned int id = getEdgeID(point.x, point.y, point.z, 2);
                    m_idToVertices.insert(ID2WPointXYZId::value_type(id, pt));
                }
                if (wMarchingCubesCaseTables::edgeTable[tableIndex] & 2048)
                {
                    WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 11);
            ...
            if (point.y == m_nCellsY - 1)
            {
                ...
                {
                    WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 1);
                    unsigned int id = getEdgeID(point.x, point.y, point.z, 1);
                    m_idToVertices.insert(ID2WPointXYZId::value_type(id, pt));
                }
                if (wMarchingCubesCaseTables::edgeTable[tableIndex] & 512)
                {
                    WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 9);
            ...
            if (point.z == m_nCellsZ - 1)
            {
                ...
                {
                    WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 4);
                    unsigned int id = getEdgeID(point.x, point.y, point.z, 4);
                    m_idToVertices.insert(ID2WPointXYZId::value_type(id, pt));
                }
                if (wMarchingCubesCaseTables::edgeTable[tableIndex] & 128)
                {
                    WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 7);
            ...
            if ((point.x == m_nCellsX - 1) && (point.y == m_nCellsY - 1))
                ...
                {
                    WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 10);
            ...
            if ((point.x == m_nCellsX - 1) && (point.z == m_nCellsZ - 1))
                ...
                {
                    WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 6);
            ...
            if ((point.y == m_nCellsY - 1) && (point.z == m_nCellsZ - 1))
                ...
                {
                    WPointXYZId pt = calculateIntersection(vals, point.x, point.y, point.z, 5);
