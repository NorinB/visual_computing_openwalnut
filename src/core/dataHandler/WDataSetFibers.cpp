...
WDataSetFibers::WDataSetFibers( WDataSetFibers::VertexArray vertices,
    ...
    : WDataSet(),
      ...
      m_bb( boundingBox )
{
    WAssert( m_vertexCoordinates->size() % 3 == 0,  "Invalid vertex array."  );
...
{
    size_t size = m_vertexCoordinates->size();
    ...
    WCreateColorArraysThread* t1 = new WCreateColorArraysThread( 0, m_lineLengths->size()/4, m_vertexCoordinates,
            m_lineLengths, globalColors, localColors, m_tangents );
    WCreateColorArraysThread* t2 = new WCreateColorArraysThread( m_lineLengths->size()/4+1, m_lineLengths->size()/2, m_vertexCoordinates,
            m_lineLengths, globalColors, localColors, m_tangents );
    WCreateColorArraysThread* t3 = new WCreateColorArraysThread( m_lineLengths->size()/2+1, m_lineLengths->size()/4*3, m_vertexCoordinates,
            m_lineLengths, globalColors, localColors, m_tangents );
    WCreateColorArraysThread* t4 = new WCreateColorArraysThread( m_lineLengths->size()/4*3+1, m_lineLengths->size()-1, m_vertexCoordinates,
            m_lineLengths, globalColors, localColors, m_tangents );
    t1->run();
    t2->run();
    t3->run();
    ...
    m_infoProperties->addProperty( "#Vertices", "The number of vertices", static_cast< WPVBaseTypes::PV_INT >( m_vertexCoordinates->size() ) );
}
...
{
    return m_vertexCoordinates->size() / 3;
}
...
void WDataSetFibers::addColorScheme( WDataSetFibers::ColorArray colors, std::string name, std::string description )
{
    ...
    size_t verts = m_vertexCoordinates->size() / 3;
...
WPosition WDataSetFibers::getPosition( size_t fiber, size_t vertex ) const
{
    ...
    index += vertex * 3;
    return WPosition( m_vertexCoordinates->at( index ), m_vertexCoordinates->at( index + 1 ), m_vertexCoordinates->at( index + 2 ) );
}
...
WFiber WDataSetFibers::operator[]( size_t numTract ) const
{
    ...
    {
        result.push_back( WPosition( ( *m_vertexCoordinates )[vIdx], ( *m_vertexCoordinates )[vIdx + 1], ( *m_vertexCoordinates )[vIdx + 2]  ) );
        vIdx += 3;
    }
    return result;
}
