    ...
    m_notColor( osg::Vec4( 0.828f, 0.391f, 0.391f, 0.5f ) )
{
    ...
    m_pickHandler3D->getPickSignal()->connect( boost::bind( &WROIBox::registerRedrawRequest3D, this, boost::placeholders::_1 ) );
...
void WROIBox::registerRedrawRequest3D( std::pair< WPickInfo3D, WPickInfo3D > pickInfo )
{
    std::unique_lock< std::shared_mutex > lock;
    lock = std::unique_lock< std::shared_mutex >( m_updateLock );
...
void WROIBox::handlePick3D()
{
    ...
    if( m_pickInfo3D.first.getName() == ss.str() )
    {
        if( m_isPicked3D )
        {
            // update color when buttons on right hand change
            if( m_pickInfo3D.second.getButtons() != m_oldPickInfo3D.second.getButtons() )
            {
                if( m_pickInfo3D.second.getButtons() & WPickInfo3D::TRIGGER )
                {
                    updateColor( osg::Vec4( 0.0f, 1.0f, 0.0f, 0.4f ) );
                }
                else
            ...
            osg::Matrix sceneMatrix = getWorldMatrices().back();

            if( m_pickInfo3D.second.getButtons() & WPickInfo3D::TRIGGER )
            {
                // resize
                osg::Vec3 maxPos = m_maxPos->get();

                maxPos = maxPos *sceneMatrix;
                maxPos += ( m_pickInfo3D.second.getPosition() - m_oldPickInfo3D.second.getPosition() );
                maxPos = maxPos * osg::Matrixd::inverse( sceneMatrix );

                m_maxPos->set( WPosition( maxPos ) );
            }
            else
            {
                // move
                osg::Vec3 sizeHalf( ( m_maxPos->get() - m_minPos->get() ) * 0.5 );
                osg::Vec3 center( m_minPos->get() );
                center += sizeHalf;
                center = center * sceneMatrix;

                center += ( m_pickInfo3D.first.getPosition() - m_oldPickInfo3D.first.getPosition() );
                ...
                m_minPos->set( WPosition( center - sizeHalf ) );
                m_maxPos->set( WPosition( center + sizeHalf ) );
            }
        }
        else
        {
            m_pickNormal = m_pickInfo3D.first.getPickNormal();
            // color for moving box
            if( m_pickInfo3D.second.getButtons() & WPickInfo3D::TRIGGER )
            {
                updateColor( osg::Vec4( 0.0f, 1.0f, 0.0f, 0.4f ) );
    ...
    if( m_isPicked3D && m_pickInfo3D.first.getName() == WPickHandler3D::unpickString )
    {
        // Perform all actions necessary for finishing a pick
        if( m_not->get() )
        {
