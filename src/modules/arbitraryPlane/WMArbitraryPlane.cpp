...
void WMArbitraryPlane::initPlane()
{
    ...
    WGraphicsEngine::getGraphicsEngine()->getScene()->insert( &( *m_s0 ) );
    WGraphicsEngine::getGraphicsEngine()->getScene()->insert( &( *m_s1 ) );
    WGraphicsEngine::getGraphicsEngine()->getScene()->insert( &( *m_s2 ) );
...
void WMArbitraryPlane::updatePlane()
{
    ...
    if( r0 != m_r0 )
    {
        osg::Quat diff = m_r0.inverse() * r0;
        osg::Matrixd mat = osg::Matrixd::translate( -osg::Vec3( p0 ) ) * osg::Matrixd::rotate( diff ) * osg::Matrixd::translate( osg::Vec3( p0 ) );
        m_r0 = r0;

        m_s1->setPosition( osg::Vec3( m_s1->getPosition() ) * mat );
        m_s2->setPosition( osg::Vec3( m_s2->getPosition() ) * mat );
    }
    WPosition p1 = m_s1->getPosition();
    WPosition p2 = m_s2->getPosition();

    // create corners of plane
    WPosition v0( p2 + ( p1 - p0 ) );
    WPosition v1( p1 + ( p0 - p2 ) );
    WPosition v2( p0 + ( p0 - v0 ) );
    WPosition v3( p2 + ( p0 - p1 ) );
