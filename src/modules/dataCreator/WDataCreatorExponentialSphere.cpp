...
#include "WDataCreatorExponentialSphere.h"
#include "core/common/WAssert.h"
#include "core/common/WLimits.h"

WDataCreatorExponentialSphere::WDataCreatorExponentialSphere():
    WObjectNDIP< WDataSetSingleCreatorInterface >( "Exponential Sphere", "Creates values exponetially decreasing from the center." )
{
    // add some properties
    m_center = m_properties->addProperty( "Center", "The center point in relative coordinates. Here the field will take its maximum",
                                          WPosition( 0.5, 0.5, 0.5 ) );
    m_radius = m_properties->addProperty( "Radius", "The radius in relative coordinates.",
                                          0.5 );
    m_radius->setMin( 0.0 );
}
...
WValueSetBase::SPtr WDataCreatorExponentialSphere::operator()( WProgress::SPtr progress,
                                                    ...
                                                    dataType /*type*/ )
{
    ...
    WAssert( ( order == 0 ) && ( dimension == 1 ), "The exponential sphere data creator only supports scalar data." );
    ...
    for( size_t x = 0; x < grid->getNbCoordsX(); ++x )
    {
        ...
        for( size_t y = 0; y < grid->getNbCoordsY(); ++y )
        {
            ...
            for( size_t z = 0; z < grid->getNbCoordsZ(); ++z )
            {
                ...
                // set value
                data->operator[]( grid->getVoxelNum( x, y, z ) ) = 1.0 / std::exp( val );
            }
