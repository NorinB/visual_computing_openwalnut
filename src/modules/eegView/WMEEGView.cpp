...
#include "core/dataHandler/WEEGSegment.h"
...
{
    m_eeg_input = std::shared_ptr< WModuleInputData< WEEG > >( new WModuleInputData< WEEG >(
...
void WMEEGView::moduleMain()
{
    ...
    while( !m_shutdownFlag() ) // loop until the module container requests the module to quit
    {
        ...
        {
            debugLog() << "EEG data or drawing mode changed";
...
void WMEEGView::redraw()
{
    ...
    if( m_eeg.get() && m_eeg->getNumberOfSegments() > 0 )
    {
        ...
        debugLog() << "  Segment " << 0;
        std::shared_ptr< WEEGSegment > segment = m_eeg->getSegment( 0 );
