...
WBresenham::WBresenham( std::shared_ptr< WGridRegular3D > grid, VoxelCompositionType compositionType, bool antialiased )
...
{
    if( m_compositionType == MAXIMUM )
    {
        return std::max( newValue, existingValue );
    }
    else if( m_compositionType == SUM )
    {
        return newValue + existingValue;
    }
    else
    {
        WAssert( false, "Unknown compositionType. Problem in implementation." );
        return 0;
    }
}
...
void WBresenham::markVoxel( const WVector3i& voxel, const int axis, const WPosition& start, const WPosition& end )
{
    ...
    {
        m_values[ idx ] = composeValue( 1.0, m_values[ idx ] );
        parameterizeVoxel( voxel, idx, axis, m_values[ idx ], start, end );
        return;
    }
